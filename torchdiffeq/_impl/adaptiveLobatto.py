...
class AdaptiveLobattoSolver(AdaptiveGridODESolver):
    ...
    def _step_func_adjoint(self, func, t0, dt, t1, y0):
        ...
        fresthalf=func(t0+0.5*(t0-t1),torch.cat((torch.reshape(torch.tensor(self.yhalf).to(y0.device, y0.dtype), y0[range(len(y0)-num_paras)].shape),torch.zeros(num_paras))))[range(-num_paras,0)]
        ...
        fresthalf_np=fresthalf.detach().cpu().numpy().reshape(-1)
        self.yhalf=np.concatenate((self.yhalf,fresthalf_np))
    ...
    def eval_estimator_adjoint(self, t0, dt, t1, y0, y1):
        ...
        self.yhalf=self.yhalf[range(len(y0)-num_paras)]
        t0_np=t0.detach().cpu().numpy().reshape(-1)
        t1_np=t1.detach().cpu().numpy().reshape(-1)
        thalf=(t0_np+t1_np)/2.
        def dtfunc(t,y):
